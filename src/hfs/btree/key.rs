...
mod tests {
    ...
    fn test_catalog_key_root_folder() {
        ...
        let key = HfsPlusCatalogKeyRaw { data: vec![0x00, 0x00, 0x00, 0x01, 0x00, 0x00] };
    ...
    fn test_catalog_key_too_short() {
        let key = HfsPlusCatalogKeyRaw { data: vec![0x00, 0x01] };
    ...
    fn test_catalog_key_partial_name() {
        ...
        let key = HfsPlusCatalogKeyRaw { data: vec![0x00, 0x00, 0x00, 0x02, 0x00, 0x05, 0x00] };
    ...
    fn test_catalog_key_unicode_name() {
        ...
        let mut data = vec![0x00, 0x00, 0x00, 0x01, (name_utf16.len() as u16).to_be_bytes()[0], (name_utf16.len() as u16).to_be_bytes()[1]];
        for &c in &name_utf16 {
            data.extend_from_slice(&c.to_be_bytes());
        }
        let key = HfsPlusCatalogKeyRaw { data };
    ...
    fn test_catalog_key_raw_roundtrip() {
        ...
        let parsed = HfsPlusCatalogKeyRaw { data: raw_key_data.to_vec() };
    ...
    fn test_extent_key_parse() {
        ...
        let raw = [0x00, 0x09, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x03];
    ...
    fn test_catalog_key_from_raw() {
        let raw = HfsPlusCatalogKeyRaw { data: vec![0x00, 0x00, 0x00, 0x07, 0x00, 0x03, 0x00, 0x61, 0x00, 0x62, 0x00, 0x63] };
