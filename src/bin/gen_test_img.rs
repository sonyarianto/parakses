...
fn main() -> anyhow::Result<()> {
    ...
    println!("Usage:");
    println!(
        "  cargo run --bin parakses -- --image {} volumes",
        out_path.display()
    );
    println!(
        "  cargo run --bin parakses -- --image {} list 0 /",
        out_path.display()
    );
    println!(
        "  cargo run --bin parakses -- --image {} cat 0 /hello.txt",
        out_path.display()
    );
    println!(
        "  cargo run --bin parakses -- --image {} extract 0 /hello.txt out.txt",
        out_path.display()
    );
